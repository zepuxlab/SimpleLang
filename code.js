...
function normalizeLayerKey(name) {
    ...
    if (LAYER_ALIASES[lower])
        return LAYER_ALIASES[lower];
    if (lower.includes("subtitle") || (lower.includes("sub") && !lower.includes("subtitled")))
        return "subtitle";
...
function parseTranslationList(raw) {
    const blocks = [];
...
function findSourceFrames() {
    ...
    for (let i = 0; i < children.length; i++) {
        ...
        const part = (_a = name.match(NAME_LOCALE_NN_LL)) === null || _a === void 0 ? void 0 : _a[0];
        if (part && DEFAULT_SOURCE_EN.test(part))
            return [children[i]];
    }
    return [];
}
function detachAllInstances(root) {
    ...
    for (let i = children.length - 1; i >= 0; i--) {
        ...
        if (child.type === "INSTANCE") {
            try {
                const detach = child.detachInstance;
...
const FILL_ORDER = ["title", "subtitle", "desc"];
function collectAllTextNodes(root) {
    const findAll = root.findAll;
    let list;
    if (typeof findAll === "function") {
        const textNodes = findAll.call(root, (n) => n.type === "TEXT");
        ...
        function traverse(node) {
            if (node.type === "TEXT") {
                list.push({ node: node, layerName: normalizeLayerKey(node.name || "") });
            }
    ...
    const y = (n) => ("y" in n && typeof n.y === "number" ? n.y : 0);
    const x = (n) => ("x" in n && typeof n.x === "number" ? n.x : 0);
    list.sort((a, b) => {
...
async function loadFontForNode(node) {
    ...
    for (let i = 0; i < len; i++) {
        const f = (_b = (_a = node).getRangeFontName) === null || _b === void 0 ? void 0 : _b.call(_a, i, i + 1);
...
async function duplicateAndFill(source, block, baseY) {
    ...
    if (duplicated.name === source.name)
        duplicated.name = (source.name.trim() || block.localeSuffix) + " " + block.localeSuffix;
...
function notifyUI(type, message) {
...
figma.ui.onmessage = async (msg) => {
    ...
    if (m.type === "saveToken") {
        try {
            const token = ((_a = m.mcpToken) !== null && _a !== void 0 ? _a : "").trim();
            if (token)
                await figma.clientStorage.setAsync(MCP_TOKEN_KEY, token);
            else
                await figma.clientStorage.deleteAsync(MCP_TOKEN_KEY);
    ...
    try {
        const gap = 40;
        const baseYByIndex = sources.map((s) => {
